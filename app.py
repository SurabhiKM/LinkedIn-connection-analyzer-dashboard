...
lottie_linkedin = load_lottieurl(
    "https://assets7.lottiefiles.com/packages/lf20_yNYxCH.json"
)
st_lottie(lottie_linkedin, speed=0.8, height=200, key="initial")

# introduction
row0_1, row0_spacer, row0_2 = st.columns([2, 1, 1])
with row0_1:
    st.image(
...
with row0_2:
    " "
    " "
    "### 🛠 by [Surabhi Khandare](https://www.linkedin.com/in/surabhi-khandare-633bb5b0/)"
...
Data you are seeing now is my (Surabhi's) connections data. If you would like to see your own \
...
df = remove_first_3_line(connections_csv)
...
with row1_1:
    lottie_connections = load_lottieurl(
        "https://assets6.lottiefiles.com/packages/lf20_fuuhilwf.json"
    )
...
with row1_2:
    lottie_company = load_lottieurl(
        "https://assets3.lottiefiles.com/packages/lf20_Sw60y11Cpf.json"
    )
...
with row1_3:
    lottie_job = load_lottieurl(
        "https://assets5.lottiefiles.com/packages/lf20_bo8vqwyw.json"
    )
...
# 2 column charts
row2_1, row_padding, row2_2 = st.columns([2, 0.15, 2])
...
with row3_1:
    ...
    shake_mask = np.array(Image.open("hand_shake.png"))
    wordcloud = (
        WordCloud(
            mask=shake_mask,
